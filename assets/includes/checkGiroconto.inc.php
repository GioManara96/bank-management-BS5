...
if (isset($_POST["sendGiroconto"])) {
    ...
    if ($query) {
        ...
        if ($saldoPreventivo > 0) {
            ...
            $stato = "In attesa";
            $tipo = 11;
            $sql2 = "INSERT INTO TMovimentiConto (ContoCorrenteID, Importo, Saldo, CategoriaMovimentoID, DescrizioneEstesa, Stato) VALUES (?, ?, ?, ?, ?, ?);";
            $stmt = mysqli_stmt_init($conn);
            mysqli_stmt_prepare($stmt, $sql2);
            mysqli_stmt_bind_param($stmt, "iddiss", $idConto, $importo, $saldoPreventivo, $tipo, $causale, $stato);
            if (mysqli_stmt_execute($stmt)) {
                ...
                if ($mail->send()) {
                    ...
                    if (mysqli_stmt_execute($stmt)) {
                        ...
                        header("location: ../../riepilogoGiroconto.php");
                        exit();
                    } else {
